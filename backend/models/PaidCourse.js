...
const paidCourseSchema = new mongoose.Schema({
  ...
  price: {
    type: Number,
    required: true,
    min: 1 // Minimum price of 1 for paid courses
  },
  category: {
    type: String,
    required: true,
...
paidCourseSchema.virtual('totalEnrolledStudents').get(function() {
  return this.studentsEnrolled.length;
});

// Virtual for total video count
paidCourseSchema.virtual('totalVideos').get(function() {
  return this.videoLinks.length;
});

// Virtual for total course duration
paidCourseSchema.virtual('totalDuration').get(function() {
...
paidCourseSchema.index({ category: 1, isActive: 1 });
paidCourseSchema.index({ courseTitle: 'text', 'courseDetails.description': 'text' });
paidCourseSchema.index({ price: 1 }); // Add index for price-based queries
